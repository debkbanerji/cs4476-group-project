...
class SeamApp(QWidget):
    ...
    def __init__(self):
        ...
        self.addShirtButton = QPushButton('Add Another Shirt')
        self.addShirtButton.clicked.connect(self.addAnotherShirt)
    ...
    def mapShirt(self, shirt):
        ...
        shirt_pts_name = 't_shirt' + str(self.countShirts) + 'Points.npy'
        pts_shirt = np.asarray(np.load(shirt_pts_name))
        shirtpoints = [pts_shirt[:, 0], pts_shirt[:, 1]]

        homography = self.computeH(shirtpoints, userpoints)
        stiched_image, sharpened_image = self.warpImage(shirt_foreground_img, shirt_img, human_img, homography)

        stichedImWidget = ImageWidget("none", "none", imageArray=stiched_image)
        sharpenedImWidget = ImageWidget("none", "none", imageArray=sharpened_image)
        hbox = QHBoxLayout()
        hbox.addWidget(stichedImWidget)
        hbox.addWidget(sharpenedImWidget)
        sharpenedImWidget.show()
        stichedImWidget.show()

        vbox = QVBoxLayout()
        vbox.addLayout(hbox)
        vbox.addWidget(self.addShirtButton)
    ...
    def warpImage(self, inputIm, image, refIm, H):
        homography = H
